...
public:
    ...
    //     ////////////////     TIMER1 Clock Source     /////////////////     //
    //!
    //! \brief      Enumeração da fonte do relógio
    //! \details    Opções de pré -escaler de relógio associadas ao Timer1.
    ...
    enum class ClockSource : uint8_t {
        DISABLED                    = 0,    //!<Desabilita o relógio, o timer está desligado
        PRESCALER_1                 = 1,    //!< Desative o Prescaler, a frequência do relógio do timer será igual ao relógio da CPU
        PRESCALER_8                 = 2,    //!< A frequência do relógio do timer será o relógio da CPU dividido por 8
        PRESCALER_64                = 3,    //!< A frequência do relógio do timer será o relógio da CPU dividido por 64
        PRESCALER_256               = 4,    //!< A frequência do relógio do timer será o relógio da CPU dividido por 256
        PRESCALER_1024              = 5,    //!< A frequência do relógio do timer será o relógio da CPU dividido por 1024
        T1_FALLING_EDGE             = 6,    //!< O relógio do timer será baseado na borda de queda de um sinal colocado no pino T1
        T1_RISING_EDGE              = 7     //!< O relógio do timer será baseado na borda ascendente de um sinal colocado no pino T1
    };

    //     ////////////////////     TIMER1 Mode     /////////////////////     //
    //!
    //! \brief      Enumeração do modo de operação
    //! \details    Opções de modo de operação associadas ao Timer1.
    //! \warning    O valor associado aos nomes simbólicos pode mudar
    //!                 entre construções e pode não refletir o registro/bits
    //!                valores reais.Para garantir a compatibilidade entre
    //!                 construir, sempre se refere ao nome simbólico, em vez de
    ...
    enum class OutputMode : uint8_t {
        NORMAL                          = 0,    //!< OC1X desativado.Operação de porta normal
        TOGGLE_ON_COMPARE               = 1,    //!< OC1X vai atingir a correspondência de comparação OCR1X
        CLEAR_ON_COMPARE                = 2,    //!< OC1X será limpo no OCR1X Compare Match
        SET_ON_COMPARE                  = 3,    //!< OC1X definirá no OCR1X Compare Match
        NON_INVERTING_MODE              = 2,    //!< OC1X produzirá um sinal PWM não invertido
        INVERTING_MODE                  = 3,    //!< OC1X produzirá um sinal PWM invertido
    };
